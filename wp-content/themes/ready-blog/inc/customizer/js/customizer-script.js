...
( function( $, api ) {
	...
	api.controlConstructor['ready-blog-repeater-text'] = api.Control.extend({
    ...
    // upsell 
    api.sectionConstructor['ready-blog-upsell'] = api.Section.extend( {
...
jQuery(document).ready(function($) {
    ...
    $('#ready-blog-img-container img').click(function() {      
        var item_id = $(this).parent().parent().parent().parent().attr('id');

        $('#' + item_id).find('li').each(function() {
            $('#' + item_id).find('img').removeClass('ready-blog-radio-img-selected') ;
        });

        $(this).addClass('ready-blog-radio-img-selected') ;
    });
